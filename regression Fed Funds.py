...
macro_path = os.path.join(base_dir, "data/macro_individual_csvs/Federal_Funds_Rate.csv")  # <-- Updated
plot_output = os.path.join(base_dir, "charts/regression_SPY_vs_FedFunds.png")             # <-- Updated
csv_output = os.path.join(base_dir, "data/regression_output_SPY_vs_FedFunds.csv")         # <-- Updated
...
plt.xlabel("Federal Funds Rate")  # <-- Updated
plt.ylabel("S&P 500 Monthly Log Returns")
plt.title("Linear Regression: SPY Returns vs Federal Funds Rate")  # <-- Updated
plt.grid(True)
plt.legend()
plt.tight_layout()
...
summary_output_path = os.path.join(base_dir, "data/regression_summary_SPY_vs_FedFunds.txt")  # <-- Updated
